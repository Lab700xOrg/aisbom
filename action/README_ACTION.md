...
4. **Package registry lookups — on by default.** Exact `requirements.txt` pins found in `directory` (package name and version only) are requested from `pypi.org` to fill in each dependency's declared license, and, when `token` is set (which turns on `--vex`), looked up in the OSV database at `api.osv.dev` for CVE statements. No SBOM content, file names or repo identifiers are sent. Separately, a model file in `directory` that sits in the Hugging Face cache layout or next to a `config.json` naming its Hugging Face repo triggers a model-card lookup on `huggingface.co`; only that Hugging Face repo id and commit are sent, never your GitHub repo, file names or hashes. Set `AISBOM_OFFLINE=1` in your workflow's `env:` block to skip all of these (or `AISBOM_NO_HF_LOOKUP=1` for the model-card lookup alone); it also disables the CLI's own telemetry events, and it cannot be combined with `share: true`, which needs the network.
